...
(ns uk.org.il2ssd.ui-init
    ...
    (:import (java.nio.file Paths)
             ...
             (uk.org.il2ssd MainView MainPresenter SingleView)))
...
(defn init-handlers []
    (let [{:keys [connect-btn
        ...
        (.. mode-choice valueProperty (addListener (event/changed-choice)))
        (.. server-path-lbl textProperty (addListener (event/changed-choice)))))
...
(defn init-controls []
    (let [{:keys [ip-field
          ...
          configuration (settings/read-from-file)]
        (.. mode-choice getItems (addAll (map state/modes [:single :cycle :dcg])))
        (if configuration
            (do (.. mode-choice getSelectionModel
                    (select
                        ((comp state/modes keyword get-in) configuration ["Mission" "Mode"] "single")))
